...
- [feedback_enum_db_value_parity.md](feedback_enum_db_value_parity.md) — Enum values in code MUST equal the exact string stored in DB — no casing transforms, no mappers
- [feedback_no_drop_table.md](feedback_no_drop_table.md) — Never DROP TABLE/SCHEMA; decommission via `@@ignore` in Prisma. Rows soft-deleted via status + archived
- [feedback_subagent_dispatch_pattern.md](feedback_subagent_dispatch_pattern.md) — Impl via Sonnet sub-agent; review via dedicated Opus 4.7 sub-agent. Main thread orchestrates only
...
- [reference_postgres_schema.md](reference_postgres_schema.md) — Confirmed PG schema v1.0 (2026-04-27): 44 tables, UUID PKs, NUMERIC(18,4), TIMESTAMPTZ, RLS, polymorphic FKs, immutability triggers
...
## References — Service & Data Architecture

- [reference_service_catalog.md](reference_service_catalog.md) — Confirmed 11-service catalog + 40-table ownership (2026-04-28). Renames: transaction→payment-processing, business+card→organization, fraud→risk; +audit-service
- [reference_per_service_prisma_packages.md](reference_per_service_prisma_packages.md) — Per-service PG schema + dedicated Prisma package under `packages/` (@repo/database-{iam,organization,payment-processing,session,audit}). Monolithic @repo/database decomposed
- [reference_prisma_dao_pattern.md](reference_prisma_dao_pattern.md) — DAO/helper pattern per @repo/database-\* package — `PrismaBaseRepository<TDelegate,TModel>`, helpers, `PrismaModule` + `PRISMA_TOKEN` DI, concrete repos extend the base
- [reference_reference_data_placement.md](reference_reference_data_placement.md) — D18 (2026-05-11): country/currency/language/MOP/service_definition live as real PG tables in @repo/database; per-service packages reference via plain String columns, no @relation
